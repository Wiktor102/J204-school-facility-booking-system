...
export class MySQLSessionStore extends session.Store {
  ...
  override async set(
    ...
    try {
      ...
      await this.pool.query(
        `REPLACE INTO ${this.tableName} (session_id, data, expires_at) VALUES (?, ?, ?)`,
  ...
  override async touch(
    ...
    try {
      const expiresAt = new Date(Date.now() + (sess.cookie?.maxAge ?? 86400000));
      await this.pool.query(
        `UPDATE ${this.tableName} SET expires_at = ?, data = ? WHERE session_id = ?`,
        [expiresAt, JSON.stringify(sess), sid],
      );
      callback();
    } catch (error) {
      callback(error);
