...
export class BookingRepository {
	...
	async create(data: {
       ...
       VALUES (?, ?, ?, ?, ?, ?)`,
			[data.userId, data.equipmentId, data.bookingDate, data.startTime, data.endTime, data.status ?? "active"]
		);
		return result.insertId;
	}

	async cancel(id: number): Promise<void> {
		await this.pool.query(`UPDATE bookings SET status = 'cancelled', cancelled_at = NOW() WHERE id = ?`, [id]);
	}

	async findById(id: number): Promise<Booking | null> {
		const [rows] = await this.pool.query<(BookingRow & RowDataPacket)[]>("SELECT * FROM bookings WHERE id = ? LIMIT 1", [
			id
		]);
		if (!rows.length) {
			return null;
