...
export class DashboardController {
  ...
  show = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      let bookingCount = 0;
      let recentBookings: Array<{ equipmentName: string; bookingDate: Date; startTime: string }> =
        [];
      if (user) {
        const bookings = await this.bookingService.listUserBookings(user.id);
        bookingCount = bookings.filter(
          (b) => b.status === BookingStatus.ACTIVE && !b.isCompleted,
        ).length;
  ...
  private async loadUpcomingSlots(
    ...
    for (let offset = 0; offset < 7 && slots.length < 3; offset++) {
      ...
      for (const slot of daySlots.slots) {
        if (slot.isAvailable) {
          slots.push({ date: formatted, startTime: slot.startTime, endTime: slot.endTime });
        }
        if (slots.length === 3) {
          break;
        }
